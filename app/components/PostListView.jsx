...
export function PostCard({ post }) {
    return <Link href={`/posts/${post?.id}`}>
        <div className="flex flex-col gap-3 p-5 rounded shadow-lg transition-all duration-700 hover:scale-110">
            <div className="relative">
                <div className="absolute flex justify-end w-full p-3 text-white">
...
async function AuthorCard({authorId}){
    ...
    return <div className="flex gap-2 items-center">
        <img className="h-6 w-5 rounded-full object-cover" src={author?.photoURL} alt="" />
